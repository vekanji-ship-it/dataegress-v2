      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <h1 className="text-5xl font-extrabold tracking-tight text-slate-900 mb-6">
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 to-blue-600">自動化串接</span>，讓工具替你工作
          </h1>
          <p className="text-slate-500 max-w-2xl mx-auto">用 Make.com 把工具串起來，重複的事情讓它自動跑。</p>
        </div>
          ...
          {stackTools.map((tool) => (
            <div key={tool.id} className="bg-white/80 backdrop-blur-sm rounded-[32px] shadow-lg border border-slate-200 p-8 hover:-translate-y-1 hover:shadow-xl transition-all duration-300 flex flex-col justify-between">
              <div>
                ...
                <p className="text-slate-600 mb-6 leading-relaxed line-clamp-3">{tool.tagline}</p>
              </div>
              <div className="flex flex-col gap-3 mt-auto">
                ...
                <Link href={tool.affiliateUrl} target="_blank" rel="noopener noreferrer" className="w-full text-center font-bold text-white bg-gradient-to-r from-indigo-500 to-blue-600 hover:shadow-lg hover:shadow-indigo-200 px-6 py-3 rounded-xl transition-all active:scale-95">
                  免費試用 {tool.name}
                </Link>
                {tool.slug && (
